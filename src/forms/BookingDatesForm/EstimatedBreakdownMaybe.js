...
import { LINE_ITEM_DAY, LINE_ITEM_NIGHT, LINE_ITEM_UNITS, LINE_ITEM_CLEANING_FEE, DATE_TYPE_DATE } from '../../util/types';
...
const estimatedNightlyPrice = (unitPrice, unitCount) => {
...
const estimatedTransaction = (unitType, bookingStart, bookingEnd, unitPrice, quantity, cleaningFee) => {
  const now = new Date();
  ...
  const totalPrice = estimatedTotalPrice(unitPrice, unitCount, cleaningFee);
  ...
  const lineItems = [
    ...cleaningFeeLineItemMaybe,
    {
      ...
      lineTotal: estimatedNightlyPrice(unitPrice, unitCount),
      reversal: false,
    },
  ];
